...
ROFI_PROMPT="Set Chromium default"
...
mkdir -p "$CONFIG_DIR"
tmp_file=$(mktemp)
jq -n --arg name "$profile_name" --arg dir "$profile_dir" '{name: $name, dir: $dir}' > "$tmp_file"
mv "$tmp_file" "$DEFAULT_PROFILE_FILE"
